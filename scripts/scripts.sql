...
----------------------------------------------------------------------------
-- Get average user years for all apps in both stores
-- new -- 
SELECT z.avg_user_years
INTO temp table alldata
FROM (
	  SELECT  ---- AVG(star_years * longevity) as avg_user_years for all common apps from both stores	           	  
       CAST(AVG( 
              ...
              (cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0)) * p.rating))/
               ((cast(a.review_count as numeric(10,0)) * 25.5) + 
                 cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0))) * 
                  (cast(a.review_count as numeric(10,0)) * 25.5) + 
                   cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0)),0) * 
					   
			  (TRUNC((ROUND(((a.rating * (cast(a.review_count as numeric(10,0)) * 25.5)) + 
                (cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0)) * p.rating))/
                 ((cast(a.review_count as numeric(10,0)) * 25.5) + 
                   cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0)))*2,0)/2),1) * 2) + 1) as numeric(12,2)) as  avg_user_years
...
SELECT * FROM alldata --1,061,854,675.67
...
WITH x AS (
        ...
        (TRUNC((ROUND(((a.rating * (cast(a.review_count as numeric(10,0)) * 25.5)) + 
	      ...
	      CAST(((((TRUNC((ROUND(((a.rating * (cast(a.review_count as numeric(10,0)) * 25.5)) + 
            (cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0)) * p.rating))/
            ((cast(a.review_count as numeric(10,0)) * 25.5) + 
            cast(replace(replace(p.install_count,'+',''),',','') as numeric (10,0)))*2,0)/2),1) * 2) + 1) * 12) * 2500) as numeric(12,2)) as total_gross_profit,
